...
namespace Canvas
{
  ...
  {
    public static implicit operator Vector2(UnitPoint p) => new Vector2(p.X, p.Y);

    public static implicit operator UnitPoint(Vector2 p) => new UnitPoint(p.X, p.Y);

    public static implicit operator Vector3(UnitPoint p) => new Vector3(p.X, p.Y, 0);

    public static implicit operator UnitPoint(Vector3 p) => new UnitPoint(p.X, p.Y);
  ...
  {
    public static bool PointInText(Text text, UnitPoint tp, float tpThresHold)
    {
      return PointInText(text, new PointF((float)tp.X, (float)tp.Y), tpThresHold);
    }

    public static bool PointInText(Text text, PointF tp, float tpThresHold)
    {
      if (tp.IsEmpty)
        return false;
      Size textsize = TextRenderer.MeasureText(text.Value, text.TextFont);
      PointF textpos = new PointF((float)text.Position.X, (float)text.Position.Y);
      RectangleF hitbox = new RectangleF(tp, textsize);
      if (hitbox.Contains(textpos))
      {
        return true;
      }
    ...
    public static bool IsPointInPolygon(UnitPoint pnt, UnitPoint[] poly)
    {
      int i, j;
    ...
    public static UnitPoint[] RotateRectangle(RectangleF rectangleToRotate, UnitPoint centerPoint, double angleInDegrees)
    {
      List<UnitPoint> pointFs = new List<UnitPoint>();

      UnitPoint BL = RotatePoint(new UnitPoint(rectangleToRotate.Location.X, rectangleToRotate.Location.Y), centerPoint, angleInDegrees);
      UnitPoint TL = new UnitPoint(rectangleToRotate.Location.X, rectangleToRotate.Location.Y + rectangleToRotate.Height);
      TL = RotatePoint(TL, centerPoint, angleInDegrees);
      UnitPoint TR = new UnitPoint(rectangleToRotate.Location.X + rectangleToRotate.Width, rectangleToRotate.Location.Y + rectangleToRotate.Height);
      TR = RotatePoint(TR, centerPoint, angleInDegrees);
      UnitPoint BR = new UnitPoint(rectangleToRotate.Location.X + rectangleToRotate.Width, rectangleToRotate.Location.Y);
      BR = RotatePoint(BR, centerPoint, angleInDegrees);
    ...
    public static UnitPoint RotatePoint(UnitPoint pointToRotate, UnitPoint centerPoint, double angleInDegrees)
    {
    ...
    public static bool IsPointInRectangle(UnitPoint point, RectangleF rectangle)
    {
      if (rectangle.Contains((float)point.X, (float)point.Y))
      {
        return true;
      }
      else
    ...
    public static bool LineIntersectWithPolygon(UnitPoint lp1, UnitPoint lp2, List<PointF> points)
    {
      List<Vector2> vectorlist = new List<Vector2>();
      foreach (PointF p in points)
      {
        Vector2 vector = new Vector2(p.X, p.Y);
        vectorlist.Add(vector);
      }
      if (netDxf.MathHelper.IsPointInPolygon(new Vector2(lp1.Point.X, lp1.Point.Y), vectorlist.ToArray()))
        return true;
      if (netDxf.MathHelper.IsPointInPolygon(new Vector2(lp2.Point.X, lp2.Point.Y), vectorlist.ToArray()))
        return true;

      for (int i = 0; i < vectorlist.Count; i++)
      {
        Vector2 p1 = vectorlist[i];
        Vector2 p2 = vectorlist[0];
        if (vectorlist.Count - 1 < i)
        {
          p2 = vectorlist[i + 1];
        }
        // the rectangle bottom is top in world units and top is bottom!, confused?
        // check left
        UnitPoint p3 = new UnitPoint(p1.X, p1.Y);
        UnitPoint p4 = new UnitPoint(p2.X, p2.Y);
  ...
  public class _SelectionRectangle
  {
    private PointF m_point1;
    private PointF m_point2;

    public _SelectionRectangle(PointF mousedownpoint)
    {
  ...
  public class SelectionShape
  {
    ...
    public SelectionShape(PointF mousedownpoint)
    {
      m_point1 = mousedownpoint;
      m_points.Add(m_point1);
      m_point2 = PointF.Empty;
    }
    ...
    public void SetMousePoint(Graphics dc, PointF mousepoint, bool lasso)
    {
      if (!m_points.Contains(mousepoint))
      {
        m_points.Add(mousepoint);
        if (!lasso)
        {
          if (m_point2 != PointF.Empty)
            XorGdi.DrawRectangle(dc, PenStyles.PS_DOT, 1, GetColor(), m_point1, m_point2);
        ...
        else
        {
          ...
          {
            PointF point1 = this.m_points[i];
            PointF point2 = point1;
            if (this.m_points.Count - 1 > i)
            {
            ...
            XorGdi.DrawLine(PenStyles.PS_DOT, 1, GetColor(), dc, (int)point1.X, (int)point1.Y, (int)point2.X, (int)point2.Y);
          }
        }
      }
    }
  ...
  public class UtilsX
  {
    ...
    public static List<PointF> GetPointsFromRectangle(RectangleF rectangle)
    {
      List<PointF> pointFs = new List<PointF>();
      pointFs.Add(new PointF(rectangle.Location.X, rectangle.Location.Y));
      pointFs.Add(new PointF(rectangle.Location.X + rectangle.Width, rectangle.Location.Y));
      pointFs.Add(new PointF(rectangle.Location.X + rectangle.Width, rectangle.Location.Y + rectangle.Height));
      pointFs.Add(new PointF(rectangle.Location.X, rectangle.Location.Y + rectangle.Height));

      return pointFs;
    }

    public static RectangleF GetRectangleFromPoints(List<PointF> pointFs)
    {
      RectangleF rectangleF;

      var minx = pointFs.Min(x => x.X);
      var miny = pointFs.Min(x => x.Y);
      var maxx = pointFs.Max(x => x.X);
      var maxy = pointFs.Max(x => x.Y);
